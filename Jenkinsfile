        ...
        stage('Security Scan') {
            steps {
                sh """
                    ...
                    docker run --rm -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy:latest image --severity CRITICAL,HIGH --ignore-unfixed ${DOCKERHUB_REPO}:${IMAGE_TAG} || true
                """
                dir('app') {
